...
{
    IReadOnlyList<Candle> Aggregate(IReadOnlyList<Candle> src, TimeSpan frame, ITimeProvider time);
}
...
public sealed class AggregationService : IAggregator
{
    public IReadOnlyList<Candle> Aggregate(IReadOnlyList<Candle> src, TimeSpan frame, ITimeProvider time)
    {
        ...
        foreach (var (begin, list) in buckets)
        {
            ...
            var end = begin + frame;
            if (!IsFullFrame(end, time.Now)) continue; // отсекаем незавершенные
    ...
    private static DateTime FloorToFrame(DateTime t, TimeSpan frame)
    {
