...
{
    Task<string> ResolveActiveAsync(string root, CancellationToken ct);
}
...
public sealed class FortsFrontResolver : IContractResolver
{
    ...
    public async Task<string> ResolveActiveAsync(string root, CancellationToken ct)
    {
        ...
        if (candidates.Count == 0)
            throw new InvalidOperationException($"Не найдено контрактов для {root}.");
        ...
        foreach (var sec in candidates)
        {
            ...
            try
            {
                var last = await _client.GetLastCandleEndAsync(sec, interval: 60, ct);
                if (last > best) { best = last; bestSec = sec; }
            }
            catch { /* пропускаем одиночные ошибки */ }
        }

        return bestSec ?? throw new InvalidOperationException($"Не удалось определить фронт для {root}.");
    }
}
