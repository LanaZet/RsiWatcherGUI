...
public sealed class MoexIssClient : IMoexClient
{
    ...
    public  MoexIssClient(HttpClient http, string engine = "futures", string market = "forts", string board = "RFUD")
    {
        ...
        _http.DefaultRequestHeaders.UserAgent.ParseAdd("rsi-watcher/solid/1.0");
        _http.Timeout = TimeSpan.FromSeconds(20);
    }

    public async Task<IReadOnlyList<Candle>> GetCandlesAsync(string secid, int interval, int limit, CancellationToken ct)
    {
        var url = $"https://iss.moex.com/iss/engines/{_engine}/markets/{_market}/boards/{_board}/" +
                  $"securities/{secid}/candles.json?interval={interval}&limit={limit}";
        ...
        int iBegin = IndexOf(cols, "begin"), iEnd = IndexOf(cols, "end"),
            ...
            iLow   = IndexOf(cols, "low"),   iClose=IndexOf(cols, "close"),
            iVol   = IndexOf(cols, "volume");
        ...
        foreach (var row in data)
        {
            ...
            list.Add(new Candle(
                begin, end,
                ParseDouble(a[iOpen]), ParseDouble(a[iHigh]), ParseDouble(a[iLow]),
                ParseDouble(a[iClose]), ParseLong(a[iVol])
            ));
        }
    ...
    {
        var url = $"https://iss.moex.com/iss/engines/{_engine}/markets/{_market}/boards/{_board}/" +
                  $"securities/{secid}/candles.json?interval={interval}&limit=1";
        using var resp = await _http.GetAsync(url, ct);
        resp.EnsureSuccessStatusCode();
        var json = await resp.Content.ReadFromJsonAsync<JsonObject>(cancellationToken: ct);
    ...
    public async Task<IReadOnlyList<string>> ListSecuritiesAsync(CancellationToken ct)
    {
        ...
        foreach (var row in data)
            if (row is JsonArray a && a[idxSecid] != null)
                list.Add(a[idxSecid]!.ToString());
        return list;
    }
    ...
    {
        // Оставляем "как есть" (биржевое локальное) — НЕ конвертируем в UTC
