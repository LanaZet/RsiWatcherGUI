...
public sealed record RsiMonitorSettings(string InstrumentRoot, string? ManualSecid, bool UseAutoFront, int PollSeconds, Dictionary<Timeframe, TfConfig> Frames);
...
public sealed class RsiMonitor
{
    ...
    {
        var secid = s.UseAutoFront ? await _resolver.ResolveActiveAsync(s.InstrumentRoot, ct) : (s.ManualSecid ?? throw new InvalidOperationException("SECID not set."));
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                foreach (var (tf, cfg) in s.Frames.OrderBy(x => x.Key))
                {
                    ...
                    {
                        if (val >= cfg.OB && zones[tf] != Zone.Overbought) { zones[tf] = Zone.Overbought; await _alerts.NotifyAsync($"{secid} TF{(int)tf}m: Перекуплен (RSI={val:F2} >= {cfg.OB})", ct); }
            ...
            catch (Exception ex) { _log.Error("Цикл: " + ex.Message); }
